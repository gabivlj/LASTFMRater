...
function PlaylistShowcase({ playlists, authentified, ...props }) {
  const messages = {
    errorNoPlaylists: authentified ? (
      <h2>
        {`You haven't got any playlists yet... `}
        <Link to="/playlist/create">Wanna create one?</Link>
      </h2>
    ) : (
      <h2>This user hasn't got any playlists yet...</h2>
    ),
    playlistsSuccess: authentified ? (
      <Link className="btn btn-primary" to="/playlist/create">
        Create
      </Link>
    ) : (
      <Link className="btn btn-primary" to="/playlist/create">
        Wanna create a playlist like this user?
      </Link>
    )
  };
  let render;
  ...
  return (
    <div style={{}}>
      <h2>Playlists:</h2>
      <ul className="mt-3 mb-3 list-group">{render}</ul>
